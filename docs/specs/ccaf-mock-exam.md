...
**4.4 Exactly `select` correct answers** — every item declares a `select:` count (1, 2, or 3) and has exactly that many defensible correct options among A–D; every remaining option is a plausible-but-wrong distractor. A full mock is 45 items at `select: 1`, 11 at `select: 2`, and 4 at `select: 3` — 25% multiple-response, matching the real exam's item mix. Multiple-response items state their count in the stem and are scored all-or-nothing. Sources: [1, 4, 6]. _(Revised 2026-08-10: was "Single correct answer — exactly one defensible correct option"; exam guide v1.0 specifies multiple-choice **and** multiple-response items.)_
