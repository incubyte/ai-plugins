...
- `data/ccaf-question-bank.md` — 30 self-authored reference questions, one per task statement, each tagged with its `task:` and `select:`; style/difficulty/format anchors only, never served in an exam (helper-enforced). Seven are multiple-response so that format has anchors too.
...
- Even now the suite can occasionally report spurious failures on Windows when a fork fails inside a
  ...
  Re-run before investigating; if it reproduces, it is real. A clean run is `119 passed, 0 failed`.
