...
Catalog every distinct fact across the description, every comment, the history (when read), and any linked issues. Each fact gets one category. The category drives where it lands in the template.
...
| **Status updates from comments** | Progress notes, blockers, handoff context, "we paused this for a week", "Bob took it over". Summarize these into the description. Do NOT restate the issue's current status, assignee, or priority — those are visible in the tracker UI. |
...
| **Track dependencies** | When the issue is blocked on or by external parties, name them in an Open Blockers section. One blocker is a sentence. Three or more is a table. |
...
| **Context over assumption** | A reader with no prior knowledge should be able to read the refined issue and understand it without chasing external sources. Add the one sentence of background that makes the rest make sense. |
...
| **No sidebar restate** | Status, priority, issue type, parent, assignee, reporter, labels, components, area path, iteration — all live in the tracker UI sidebar. Do not duplicate them in the body. The exception is comment-thread context, which is hidden behind a tab and worth surfacing. |
