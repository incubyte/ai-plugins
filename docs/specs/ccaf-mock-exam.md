...
| --- | ----------------------------------------------------------------- | -------------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | CCAF exam guide (private reference - redacted)                    | Anthropic, PBC | 2025-02-10 | Private reference consulted during early drafting; not reproduced in this repo. As of 2026-06-10 every shipped artifact is self-authored or publicly corroborated (see [2] and the changelog) - the blueprint syllabus, scope lists, scenario frames, case briefs, and all 12 bank questions were re-authored from public documentation and original work. |
...
| 6   | CCAF exam guide v1.0 (revised; private reference — not reproduced) | Anthropic, PBC | 2026-07    | Exam code `CCAR-F`, effective July 2026. Published facts consulted for the 2026-08-10 revision: item formats (multiple-choice **and** multiple-response, each item stating how many responses to select), the 30-task-statement objective index (D1.1–D5.6), the scenarios' backend tool names and targets, percent-correct-by-domain reporting, criterion-referenced scoring via a standard-setting study, and program policies (fee, validity, retake waits, recertification). Consulted only; **no prose or exam item from it appears in this repo** — every shipped stem, option, explanation, brief, task-statement description, and exercise is self-authored. The PDF is gitignored. |
...
**4.4 Single correct answer** — every item has exactly one defensible correct option among A–D; the other three are plausible-but-wrong distractors. Sources: [1, 4]. _(Revised 2026-08-10 to add multiple-response items per exam guide v1.0, then reverted the same day at Dinesh's direction: the plugin serves single-answer items only. This is a **known divergence** from the guide, which specifies multiple-choice **and** multiple-response — recorded in the blueprint's "Deliberate divergence" section and the README fidelity table, together with its consequence that a mock score is if anything optimistic.)_
...
**4.6 Bank questions are curated, stable, and reference-only** — the 12 self-authored questions are version-controlled and not regenerated or re-keyed at runtime, and they serve **only** as few-shot style/difficulty anchors: no bank question (or near-verbatim variant) appears in an assembled exam, enforced at the state-helper layer. Rationale: the bank ships in the repo with answers, so serving it would inflate scores. Sources: [5]. _(Revised 2026-06-10: was "seeds appear in exams as anchors"; now reference-only, every served question is generated.)_
...
- 2026-06-10 — Dinesh — **Bank questions are now reference-only** (revises invariant 4.6; supersedes AC 5.2.4). The 12 self-authored questions no longer appear in assembled exams — they anchor style/difficulty/distractor construction only, and **all 60 questions are generated fresh per attempt** (each independently verified). Rationale: the bank ships in the repo with answers and explanations, so any candidate can read it; serving those questions inflates the readiness signal the 720 gate exists to protect. Enforcement is deterministic: `init`/`score` reject any exam containing `source: authored` or `id: seed-*`. Descriptions synced (plugin.json, marketplace.json, README, command, skill, blueprint, bank header). Suite green (76/76).
- 2026-06-10 — Dinesh — **Content refresh: blueprint and reference bank re-authored as fully original material.** The blueprint now separates publicly corroborated exam mechanics (counts, weights, names, scoring) from this plugin's own material: the per-domain syllabus rewritten as our own "be able to / common mistakes" enumeration grounded in public Anthropic product documentation, scope lists self-authored, scenario frames + case-study briefs re-invented with our own specifics (e.g., `fetch_account`/`find_order`/`issue_refund`/`handoff_human`). The 12 reference questions re-authored from scratch (ids `ref-01..12`) with a broader coverage profile (all six scenarios ×2; D1×3, D2×2, D3×3, D4×2, D5×2), new testing points, and balanced keys; the helper's bank-rejection guard extended to `id: ref-*`. Source [1] in §2 generalized to a private-reference note. Suite green (76/76).
- 2026-08-10 — Dinesh — **Aligned to exam guide v1.0 (`CCAR-F`, effective July 2026)** — see new source [6]. **(1) Multiple-response items** (revises invariant 4.4): the guide specifies multiple-choice *and* multiple-response items, each stating how many responses to select, so a mock is now 45 `select: 1` + 11 `select: 2` + 4 `select: 3` (25% multiple-response, machine-enforced exactly, choose-three capped at 5). Items carry a `select:` line that survives into the key-free questions file — the administering skill needs the count without seeing the key — and `init` rejects any item whose `select:` disagrees with its `answer_key`, repeats a letter, or lists letters out of A–D order. `record` accepts a letter set in any case or order and normalizes it (uppercased, sorted, de-duplicated), so scoring is exact set equality: **all-or-nothing, no partial credit**. Multiple-response items render with `multiSelect: true`; a wrong response count is re-asked once. The per-letter key-spread guard now applies to the single-answer items only, with a band proportional to their count. Known divergence, disclosed in the README: every item has four options (A–D) because `AskUserQuestion` renders at most four, which makes a choose-three item easier than a five-option one — hence the choose-three minority. **(2) The 30 task statements are now real.** The blueprint formalizes D1.1–D5.6 with self-authored descriptions naming the exact identifiers items use (`Task`/`allowedTools`, `AgentDefinition`, `fork_session`, `PostToolUse`, `context: fork`/`allowed-tools`/`argument-hint`, Explore subagent, `@import`, `~/.claude.json`, `--json-schema`, Pydantic, state manifests, the interview pattern, Edit→Read+Write fallback). This closes a dangling reference: `ccaf-tutor` and `ccaf-check-author` already cited these codes against a blueprint that never defined them. **(3) Scenario facts corrected** — case briefs re-authored (still our own prose) around the guide's actual identifiers `get_customer`/`lookup_order`/`process_refund`/`escalate_to_human`, the 80%+ first-contact-resolution target, and four research subagents including a separate report generator; the previously invented names are gone. **(4) Bank grown 12 → 24** — twelve new self-authored anchors covering D1.2, D1.4, D1.5, D2.1, D2.4, D3.3, D3.4, D4.5, D4.6, D5.1, D5.2, D5.3, five of them multiple-response so that format has anchors; all 24 now tagged with `task:` and `select:`. Six task statements still lack an anchor (D1.7, D2.5, D3.5, D4.2, D4.4, D5.4) — flagged in the bank, not hidden. **(5) Reporting** — `score` now emits `pct=` per domain, matching the real score report's percent-correct-by-domain, presented as diagnostic-only since pass/fail is the total scaled score. **(6) New `data/ccaf-prep-guide.md`** — study routes, four hands-on exercises mapped to task statements, multiple-response answering strategy, and certification logistics (fee $125, Pearson VUE delivery, 14/30/90-day retake ladder, 12-month validity, free renewal assessment); the tutor can assign exercises at domain boundaries. **(7) Fork-cost discipline** — process creation is slow on some machines (~640 ms/fork measured on Windows + AV), so `normalize_answer` is pure bash and each validation check is a single `awk` pass; net effect is that `score` got *faster* than before this change despite the added checks. Scope lists aligned to the guide's in/out-of-scope enumerations. The guide PDF is gitignored so it can never ship with the plugin.
- 2026-08-10 — Dinesh — **Reverted to single-answer items only** (revises invariant 4.4 again, same day). Dinesh's call: "lets just keep single option questions, no need to have multiple option questions. Apart from this follow all the things from the pdf." So every item is now four options with exactly one correct, and the multiple-response machinery is removed rather than left dormant — the Clean Code pack's dead-code rule settles that (version control is the archive). Removed: the `select:` field from the payload and questions file, multi-letter keys and answer-set normalization, the 45/11/4 mix enforcement, `check_select_alignment`, the select histogram in `audit`, `multiSelect` rendering, the response-count re-ask, and the comma-join step. `normalize_answer` shrank to a case-insensitive single letter, which still earns its place: a candidate can type `c` into the free-text field. The seven multiple-response bank anchors were **rewritten** as single-answer questions on the same task statements rather than deleted, so all 30 task statements keep exactly one anchor. **This is a known divergence from the guide**, not alignment with it — recorded in the blueprint's "Deliberate divergence" section, the README fidelity table, `CLAUDE.md`, the prep guide, and the tutor (which tells learners the format exists and that they will not rehearse it here). The consequence is stated everywhere it appears: single-answer items are the easier format, so a mock score is if anything **optimistic** — 720 is the floor of readiness, not a margin. Everything else from guide v1.0 stays: the 30 task statements, corrected scenario identifiers, percent-per-domain plus per-objective reporting, the prep guide, and the logistics. One side effect worth noting: rewriting seven anchors left the bank's key letters skewed toward A, and every explanation names option letters (including English articles like "A bare numeric code (A)…"), so an automated remap was unsafe. Instead of massaging the examples, the property is now enforced — `check_key_spread` tightened from a sixth-to-a-half band to a sixth-to-a-third, and the bank carries an explicit "do not imitate these answer positions" note. Suite green (101/101), and a real 60-item assembly was accepted first try with a 15/15/15/15 key spread.
