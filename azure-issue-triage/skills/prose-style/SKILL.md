---
name: prose-style
description: 'Rewrites or audits prose to eliminate AI writing patterns and produce natural human writing. Use when the user says "AI slop", "sounds generated", "too formal", "write like a human", "fix the writing", "rewrite this", "clean up the prose", or asks for writing style guidance. Also apply proactively when generating documentation, Teams messages, Confluence pages, Azure DevOps work-item text, Jira ticket text, or any long-form prose output.'
metadata:
...
<!--
Mirror of plugins/jira-issue-triage/skills/prose-style/. The catalog and examples are identical
to the Jira plugin's copy. The Calling Convention section below references the
azure-issue-triage agent's phase numbering and HTML (not ADF) output. Edit both copies when the
writing rules change. A future release may consolidate them once Claude Code supports cross-plugin
skill sharing.
-->
...
The skill works two ways. When a user pastes text and asks for a rewrite or audit, run end to end. When the `azure-issue-triage` agent calls this skill, take the input the agent passes and return the cleaned version. The agent invokes the skill at two points in the workflow:

| Caller invocation point | Input the skill receives | Return value |
|-------------------------|--------------------------|--------------|
| Phase 2.5 (draft cleaning) | The drafted assessment comment, scope summary, or reporter follow-up question text (still in markdown shape, before HTML conversion). | The same text with prose-style rules applied. The agent uses the cleaned text both for the Phase 3 preview shown to the user and as the source for the HTML produced in Phase 4a, 4b, or 4c. |
| Phase 5 (refinement cleaning) | The refined title and description produced by `azure-work-item-refiner`. | The same content with prose-style rules applied, ready for the Phase 5 user-facing preview. |

In both contexts, the skill never posts to Azure DevOps on its own. The agent owns the `wit_add_work_item_comment` and `wit_update_work_item` calls.
