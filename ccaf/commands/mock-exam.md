---
description: Take a CCAF (Claude Certified Architect – Foundations) mock exam — 60 weighted items mixing single-answer and multiple-response, a scaled /1000 score, and a 720 readiness verdict. Resumable.
...
the case brief kept visible, machine-enforced domain weighting 27/18/20/20/15, both item formats —
45 single-answer plus 15 multiple-response ("Select TWO" / "Select THREE", scored all-or-nothing) —
and no penalty for guessing. It reports a scaled 100–1000 score with the 720 pass line plus percent
correct per domain, as the real score report does. The scaled number is an honest estimate — not
Anthropic's proprietary equating curve.
...
3. Assemble a 60-item exam (every item generated fresh, anchored to the reference bank and independently verified, with the blueprint's item-format mix), or continue an existing one.
4. Administer 4 items per screen via AskUserQuestion — multiple-response items render as multi-select — saving progress after each screen.
