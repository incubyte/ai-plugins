...
The title is the highest-leverage piece of text on an issue. It shows up in:

- Board cards and list views.
- Tracker search results.
- Slack and Teams unfurls when the issue URL is pasted.
- Email notifications and digest summaries.
- Sprint planning exports and burndown charts.
- The tracker sidebar of any other issue that links to this one.

A good title lets a teammate decide whether to open the issue without clicking through. A vague title forces them to open it just to find out, every time.
...
3. **Add the differentiator.** If there are ten SSO issues in the project, the title for this one says what makes it specific.
4. **Stay under 80 characters.** Most trackers truncate long titles on board and list views. Aim for 60 to 75 characters in the rewritten title; treat 80 as the hard ceiling.
...
Use this base pattern. Add modifiers when the issue warrants them.
...
If the original title already follows the pattern, says the area clearly, and would not gain anything from a rewrite, leave it alone. The rule is to make the title better, not to rewrite for its own sake. The diff at the agent's Phase 3 gate still shows the title even when unchanged so the user can confirm.
