...
Catalog every distinct fact across the description, every comment, the revision history (when read), and any linked work items. Each fact gets one category. The category drives where it lands in the template.
...
| **Source-system metadata** | Support escalation tables, Zendesk/Salesforce ticket fields, intake forms, customer-feedback exports, PagerDuty incident records |
| **Status updates from comments** | Progress notes, blockers, handoff context, "we paused this for a week", "Bob took it over". Summarize these into the description. Do NOT restate the work item's current State, AssignedTo, or Priority since those are visible in the UI. |
...
| **Track dependencies** | When the work item is blocked on or by external parties, name them in an Open Blockers section. One blocker is a sentence. Three or more is a table. |
...
| **Context over assumption** | A reader with no prior knowledge should be able to read the refined work item and understand it without chasing external sources. Add the one sentence of background that makes the rest make sense. |
...
| **No sidebar restate** | State, work-item type, parent, area path, iteration path, assignee, tags, and severity all live in the AzDO sidebar. Do not duplicate them in the body. The exception is comment-thread context, which is hidden behind the Discussion tab. |
